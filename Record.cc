...
void Record:: MergeGBRecords(Record* left, Record* right, OrderMaker* o)
{
		...
		int numAttsLeft = 1;
		int numAttsRight = ((int *) right->GetBits())[1] / sizeof(int) - 1;

		int numAttsToKeep = numAttsLeft + o->numAtts;
        int* attsToKeep = new int[numAttsToKeep];

        //for(int i =0;i<numAttsLeft;i++)
        attsToKeep[0] = 0;

        int startOfRight = 1;//numAttsLeft;

        int k=0;
        int m = numAttsToKeep; //numAttsRight+numAttsLeft;

        for(int i=numAttsLeft; i<m;i++)
        attsToKeep[i] = o->whichAtts[i-1];
...
void Record::MergeRecords(Record *left, Record *right, int numAttsLeft, int numAttsRight, int *attsToKeep, int numAttsToKeep, int startOfRight) {
    ...
    if (bits == NULL) {
        cout << "Merge Records : ERROR : Not enough memory. EXIT !!!\n";
        exit(1);
    }
